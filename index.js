(async () => {
  ...
  // 2 — Plant
  // 3 — Cow
  const availableMaps = [0, 1, 2, 3];
  // Delay before moving to the next map [min, max]
  const delayNextMap = [2 * 1000, 5 * 1000];
  // Delay after map selection [min, max]
  const delayAfterMapSelect = [2 * 1000, 4 * 1000];
  // Delay after mine [min, max]
  const delayAfterMine = [20 * 1000, 30 * 1000];
  // Delay after repair [min, max]
  const delayAfterRepair = [20 * 1000, 30 * 1000];
  ...
  while (1) {
    try {
      ...
      for (let mapId = 0; mapId < 4; ++mapId) {
        if (!availableMaps.includes(mapId)) continue;
        ...
        for (const [, item] of document
          ...
          if (currentEnergy < 400 && currentFish > 1) {
            ...
            if (countEnergyClicks > 0) {
              ...
              await new Promise((res) =>
                setTimeout(res, random(15, 20) * 1000)
              );
            }
          }
          // Restore energy end
          ...
          if (mapId === 0) {
            ...
            if (!mineLevelDiv) {
              console.log("Member click: " + document.querySelector(
              ".info-section .info-title-name").innerText);
              buttonMine.click();
            } else {
              const mineLevels = mineLevelDiv.innerText.split("/");
              if (mineLevels[0] === mineLevels[1]) {
                console.log("Tool click: " + document.querySelector(
                            ".info-section .info-title-name").innerText
            ...
            console.log("Crop/Animal click: " + document.querySelector(
                            ".info-section .info-title-name").innerText);
            buttonMine.click();
          } else {
